...
app.get("/Dashboard", async (req, res) => {
  if (req.session.user && req.cookies.user_sid) {
    ...
    console.log(
      "Auto LogOut Time: -" +
        req.session.cookie.expires.toLocaleString("en-US", {
...
app.post("/buyOrder/:stockID", async (req, res) => {
  ...
  if (bOrders[0] && sOrders[0]) {
    ...
    else res.redirect("/stock/" + stockID);
  } else res.redirect("/stock/" + stockID);
});
...
app
  ...
  .post((req, res) => {
    ...
    const url =
      "https://main.pcc.events/centralized/" + userID + "/" + password;
...
app.get("/transaction/:stockID", async (req, res) => {
  ...
  if (bOrders[0].price >= sOrders[0].price) {
    ...
    try {
      ...
      console.log(
        "Update Seller Hold ID,Quantity,sellerAvgPrice " +
          sUID +
          " " +
          sellerQ +
          " " +
    ...
    // add transaction
    await dbFunct.storeTransactionHistory(
      bUID,
      sUID,
      stockID,
      oQ,
...
app.get("/news", async (req, res) => {
  ...
  eventsRaw.forEach((event) => {
    ...
    const timeStamp = eDate.toLocaleDateString("en-us", {
      weekday: "long",
      year: "numeric",
      month: "short",
      day: "numeric",
